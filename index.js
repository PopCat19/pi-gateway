...
async function writeRuntimeModels(agentDir, ctx) {
	...
	try {
		...
		await writeFile(path.join(dir, "runtime-models.json"), JSON.stringify(models));
	} catch (err) {
		...
		handler: async (input, ctx) => {
			...
			if (subcommand === "start") {
				...
				try {
					...
					if (newStatus?.phase === "running") {
						sendText(
							pi,
							["Gateway daemon started.", `PID: ${newStatus.pid}`, `URL: ${newStatus.url}`, `Endpoints: /v1/chat/completions, /v1/models`].join(
								"\n",
							),
						);
					} else {
			...
			if (subcommand === "status") {
				...
				if (!status) {
					...
					sendText(
						pi,
						[`Gateway daemon (pid ${status.pid || "unknown"}) is not running.`, `Last phase: ${status.phase}`, `Config: ${paths.configPath}`].join(
							"\n",
						),
					);
				} else {
